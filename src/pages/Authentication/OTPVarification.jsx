...
const Signin = () => {
  const inputs = useRef([]);
  // Use state to hold the values of the 6 input fields
  const [otpValues, setOtpValues] = useState(Array(6).fill(''));

  const handleInput = (e, index) => {
    const value = e.target.value;
    // Only allow a single numeric character (0-9), replace anything else with ''
    const sanitizedValue = value.replace(/[^0-9]/g, '').slice(0, 1);

    // Update the state with the new value for the current input
    ...
    // Move to the next input if a valid number is entered and not the last input
    if (sanitizedValue.length === 1 && index < inputs.current.length - 1) {
  ...
  useEffect(() => {
    // Ensure inputs.current is correctly initialized for 6 inputs
    inputs.current = inputs.current.slice(0, 6);
  }, []);

  const handleSubmit = (e) => {
    e.preventDefault();
    // You can access the complete OTP value from otpValues state
    const completeOtp = otpValues.join('');
    console.log('Entered OTP:', completeOtp);
          ...
          <Card.Body className="p-5">
            <div className="text-center mb-4">
              <div className="logo mb-3">
                {/* Your SVG logo code remains here */}
              ...
              <h4 className="mb-1 fw-semibold">Enter the verification code</h4> {/* Fixed typo */}
              <p className="text-muted small">AN SMS containing 6-digits verification code <br />has been sent to the phone number <br /><strong>*****567</strong></p> {/* Fixed typo */}
            </div>

            <Form onSubmit={handleSubmit}>
              <Form.Group className="mb-4" controlId="formOtp"> {/* Changed controlId */}
                <div className="input-container">
                  {otpValues.map((value, index) => (
                    <React.Fragment key={index}>
                      ...
                      />
                      {/* Add the hyphen after the third input */}
                      {index === 2 && <span className="operator text-xl mx-1">-</span>}
                    </React.Fragment>
                  ))}
                </div>
              </Form.Group>
              ...
              <div className="text-center mt-4 d-flex justify-content-center">
                <a href="/login" className="small text-purple text-decoration-none" style={{ fontSize: '13px', fontWeight: '600' }}>Don't receive the code? Contact us</a>
              </div>
            </Form>
          </Card.Body>
        </Card>
